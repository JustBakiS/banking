...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESCAPE
        $(".background").fadeOut()
        $.post('https://banking/closeapp', JSON.stringify({}));
        break;
    }
});

$("#hzlcek100").click(function(){
    $.post('https://banking/cek', JSON.stringify({
        para: 100
    }));
});
...
$("#hzlyatir100").click(function(){
    $.post('https://banking/yatir', JSON.stringify({
        para: 100
    }));
});
...
$(document).on('click', '.payinvoice', function(event){
    $.post('https://banking/PayInvoice', JSON.stringify({
        invoiceId: this.id
    }));
});
